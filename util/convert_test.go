...
func TestMsgGasLimitChecker(t *testing.T) {
	type MsgGasLimitCheckerTest struct {
		name      string
		input     string
		output    *big.Int
		outputErr error
	...
	testcases := make([]MsgGasLimitCheckerTest, 0)
	testcases = append(testcases, MsgGasLimitCheckerTest{
		name:      "valid string as input",
		input:     "2000000",
		output:    big.NewInt(2000000),
		outputErr: nil,
	})
	testcases = append(testcases, MsgGasLimitCheckerTest{
		name:      "valid string as input",
	...
	testcases = append(testcases, MsgGasLimitCheckerTest{
		...
		input:     "1a2b3c",
		output:    nil,
		outputErr: errors.New("failed to convert from string to bigInt"),
	})
	testcases = append(testcases, MsgGasLimitCheckerTest{
		name:      "invalid string as input",
		input:     "-10",
		output:    nil,
		outputErr: errors.New("MsgGasLimit can't be negative number"),
	})

	for _, testcase := range testcases {
		re, err := util.MsgGasLimitChecker(testcase.input)
		assert.EqualValues(t, testcase.output, re, testcase.name)
		assert.EqualValues(t, testcase.outputErr, err, testcase.name)
	}
}
